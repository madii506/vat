...
export function createVat(canvas, { transparent = true, dpr = 1.5, dist = 12.5 } = {}) {
  ...
  const camera = new THREE.PerspectiveCamera(30, 1, .1, 100); camera.position.set(0, 1.1, dist); camera.lookAt(0, 0, 0);
  scene.add(new THREE.HemisphereLight(0xfffdf8, 0xd9d4c8, .9));
  ...
  root.add(glass);
  const bottom = new THREE.Mesh(new THREE.CylinderGeometry(R, R, .18, 96), new THREE.MeshStandardMaterial({ color: 0x2a2724, roughness: .4 })); bottom.position.y = -H / 2 - .09; root.add(bottom);
  const base = new THREE.Mesh(new THREE.CylinderGeometry(R + .35, R + .5, .35, 96), new THREE.MeshStandardMaterial({ color: 0x2e2a25, roughness: .35, metalness: .2 })); base.position.y = -H / 2 - .35; root.add(base);
  const lid = new THREE.Mesh(new THREE.CylinderGeometry(R + .16, R + .04, .4, 96), new THREE.MeshStandardMaterial({ color: 0x2e2a25, roughness: .35, metalness: .2 })); lid.position.y = H / 2 + .2; root.add(lid);
  ...
  function resize() { const w = canvas.clientWidth || 800, h = canvas.clientHeight || 600; renderer.setSize(w, h, false); camera.aspect = w / h; camera.updateProjectionMatrix(); camera.position.z = w < 520 ? dist + 3 : dist; }
